...
export class HomeChangePasswordComponent {
  ...
  onSubmit(form: FormGroup) {
    ...
    // Encrypt password using SHA256 before sending to server
    const encryptedPassword = CryptoJS.SHA256(form.value.newPass).toString();
    
    this.userService
      .changePassword(this.currentUser._id, encryptedPassword)
      .subscribe((res) => {
        if (res["status"] == 1) {
